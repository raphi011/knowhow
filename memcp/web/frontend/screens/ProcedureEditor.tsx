...
const ProcedureEditor = () => {
  ...
  const [formData, setFormData] = useState<Procedure>({
    id: id || `proc-${Math.random().toString(36).substr(2, 4)}`,
    name: '',
    description: '',
    steps: [{ content: '' }],
    labels: [],
  ...
  useEffect(() => {
    if (id) {
      backend.getProcedure(id).then(res => {
  ...
  const handleSave = async () => {
    ...
    await backend.saveProcedure(formData);
    ...
    <div className="flex flex-col h-full animate-page-in overflow-hidden">
      <header className="flex items-center justify-between px-6 py-4 border-b border-[#233f48] bg-background-dark/80 backdrop-blur-md sticky top-0 z-30">
        <div className="flex items-center gap-4">
          <Link to="/procedures" className="size-10 rounded-xl bg-surface-dark border border-[#233f48] flex items-center justify-center text-text-secondary hover:text-white transition-all">
            <span className="material-symbols-outlined">close</span>
          </Link>
          <h2 className="text-white text-xl font-black leading-none uppercase tracking-tight">
            {id ? 'Edit Procedure' : 'New Procedure Protocol'}
          </h2>
        </div>
