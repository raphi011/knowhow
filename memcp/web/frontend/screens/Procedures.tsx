...
const Procedures = () => {
  ...
  useEffect(() => {
    setLoading(true);
    backend.getProcedures(currentContext).then(res => {
      setProcedures(res);
      setLoading(false);
    });
  }, [currentContext]);
  ...
  return (
    <div className="flex flex-col h-full animate-page-in overflow-hidden">
      <header className="flex items-center justify-between px-6 py-4 border-b border-[#233f48] bg-background-dark/80 backdrop-blur-md sticky top-0 z-30">
        ...
        <Link to="/procedure/new" className="bg-primary hover:bg-primary-dark text-[#101d22] font-black px-6 py-2 rounded-xl text-xs uppercase tracking-widest flex items-center gap-2 transition-all shadow-xl shadow-primary/20">
                ...
                <div key={proc.id} className="bg-surface-dark border border-[#233f48] rounded-2xl p-6 flex flex-col gap-6 shadow-xl hover:border-primary/30 transition-all group">
                   <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2">
                        <Link to={`/procedure/edit/${proc.id}`} className="size-8 rounded-lg bg-[#233f48] border border-white/5 flex items-center justify-center text-text-secondary hover:text-white">
