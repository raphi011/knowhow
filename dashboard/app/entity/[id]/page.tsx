...
export default function EntityDetail() {
  ...
  useEffect(() => {
    if (id) {
      graphql<{ entity: EntityData }>(QUERIES.entity, { id: decodeURIComponent(id) })
        .then(res => {
          setEntity(res.entity);
          if (res.entity) setUserImportance(res.entity.importance || 0.5);
        })
      ...
      <header className="flex-none flex items-center justify-between border-b border-[#233f48] bg-background-dark/95 backdrop-blur-md px-6 py-4 sticky top-0 z-40">
        <div className="flex items-center gap-8">
          <nav className="flex items-center text-[10px] font-black uppercase tracking-[0.15em] text-text-secondary">
            <Link href="/" className="hover:text-primary transition-colors">MEMCP</Link>
            <span className="material-symbols-outlined text-[14px] mx-3 opacity-30">chevron_right</span>
            <Link href="/search" className="hover:text-primary transition-colors">STORAGE</Link>
            <span className="material-symbols-outlined text-[14px] mx-3 opacity-30">chevron_right</span>
        ...
        <div className="lg:col-span-8 flex flex-col gap-10">
          <div className="flex flex-col gap-3 border-b border-[#233f48] pb-8">
            ...
            <p className="text-text-secondary text-xs flex items-center gap-2 font-bold uppercase tracking-widest opacity-60">
              <span className="material-symbols-outlined text-[16px]">schedule</span> {entity.accessCount || 0} accesses
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {entity.neighbors.map((neighbor, idx) => (
                <Link key={idx} href={`/entity/${encodeURIComponent(neighbor.id)}`} className="p-6 rounded-2xl border border-[#233f48] bg-surface-dark transition-all cursor-pointer group shadow-sm active:scale-[0.98] hover:border-primary/50">
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 text-green-400">
                      <span className="size-1.5 rounded-full shadow-[0_0_8px_rgba(74,222,128,0.5)] bg-green-400"></span> {neighbor.type}
                    </span>
                    <span className="text-[9px] font-mono font-bold text-text-secondary opacity-40">{neighbor.id}</span>
                  </div>
                  <p className="text-sm font-bold text-slate-300 transition-colors leading-relaxed group-hover:text-primary">{neighbor.content}</p>
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="p-6 bg-black/30 rounded-2xl border border-[#233f48] space-y-6 shadow-inner">
              <div className="space-y-3">
                <div className="flex justify-between items-center text-[10px] uppercase font-black tracking-widest">
                  ...
                  <span className="text-primary font-mono font-black">{(entity.importance || 0).toFixed(2)}</span>
                </div>
                <div className="h-1.5 w-full bg-[#101d22] rounded-full overflow-hidden">
                  <div className="h-full bg-primary shadow-[0_0_10px_rgba(19,182,236,0.5)]" style={{ width: `${(entity.importance || 0) * 100}%` }}></div>
                </div>
              </div>
