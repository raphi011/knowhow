...
export default function EpisodeDetail() {
  ...
  useEffect(() => {
    if (id) {
      graphql<{ episode: Episode }>(QUERIES.episode, { id: decodeURIComponent(id) })
    ...
    <div className="flex flex-col h-full animate-page-in overflow-hidden">
      <header className="flex items-center justify-between px-6 py-4 border-b border-[#233f48] bg-background-dark/80 backdrop-blur-md sticky top-0 z-30">
        <div className="flex items-center gap-4">
          <Link href="/episodes" className="size-10 rounded-xl bg-surface-dark border border-[#233f48] flex items-center justify-center text-text-secondary hover:text-white transition-all">
            <span className="material-symbols-outlined">arrow_back</span>
          </Link>
          <div className="flex flex-col">
            <h2 className="text-white text-xl font-black leading-none uppercase tracking-tight">Episode: {episode.id}</h2>
            <p className="text-text-secondary text-xs mt-1 font-medium">{episode.timestamp ? new Date(episode.timestamp).toLocaleString() : ''}</p>
          </div>
        </div>
            ...
            <div className="bg-surface-dark rounded-2xl border border-[#233f48] p-6 space-y-6 shadow-xl">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="text-white font-bold">{(episode.metadata as { tokens?: number })?.tokens || 0}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-text-secondary font-medium">Access Count</span>
                  <span className="text-white font-bold">{episode.accessCount || 0}</span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-[10px] text-slate-300 font-bold">{(episode.metadata as { agent?: string })?.agent || 'Unknown'}</span>
                </div>
              </div>
            </div>
