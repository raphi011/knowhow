...
import { graphql, QUERIES, MUTATIONS } from '@/lib/graphql';
...
export default function EditProcedure() {
  ...
  useEffect(() => {
    if (id) {
      graphql<{ procedure: Procedure }>(QUERIES.procedure, { id: decodeURIComponent(id) })
        .then(res => {
          if (res.procedure) setFormData(res.procedure);
        })
        .catch(() => {});
    }
  }, [id]);
